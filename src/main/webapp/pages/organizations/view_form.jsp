...
<%--        <input hidden value="${forms.id}" name="id">--%>
        <form action="#">
          <div class="row">
            <div class="row">
              <h4 class="col-md-0 mb-4">
                <c:forEach var="forms" items="${forms}">
                  <div class="border p-3 my-3 rounded">
                    <div class="d-flex align-items-center mb-2">
                      <h5 class="text-extra-justify">${forms.name_forms}</h5>
                    </div>
                    <c:forEach var="answer" items="${question.answerList}">
                      <div class="form-check">
                        <input class="form-check-input" type="radio" name="radio${question.form_id}" id="radio${question.form_id}Option${answer.id_question}"
                               value="${answer.answer}" required>
                        <label class="form-check-label text-extra-justify" for="radio${question.form_id}Option${answer.id_question}">
                            ${answer.answer}
                        </label>
                      </div>
                    </c:forEach>
